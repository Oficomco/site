...
class Mage_Checkout_Block_Onepage_Success extends Mage_Core_Block_Template {
	...
	protected function _prepareLastBillingAgreement() {
		...
		if ($agreementId && $customerId) {
			...
			if ($agreement -> getId() && $customerId == $agreement -> getCustomerId()) {
				$this -> addData(array('agreement_ref_id' => $agreement -> getReferenceId(), 'agreement_url' => $this -> getUrl('sales/billing_agreement/view', array('agreement' => $agreementId)), ));
			}
		}
	}
	...
	protected function _prepareLastRecurringProfiles() {
		...
		if ($profileIds && is_array($profileIds)) {
			$collection = Mage::getModel('sales/recurring_profile') -> getCollection() -> addFieldToFilter('profile_id', array('in' => $profileIds));
			$profiles = array();
	...
	public function sendToNetsuite() {
		$orderId = Mage::getSingleton('checkout/session') -> getLastOrderId();
		if ($orderId) {
			$order = Mage::getModel('sales/order') -> load($orderId);
	        ...
	        $helper->sendOrderToNetsuite($order);
	...
	public function getTransaction() {
		...
		$trans = array('id'=>"$orderId", 'affiliation'=>'Oficomco','revenue'=>"$amount", 'shipping'=>"$shipping", 'tax'=>"$tax");
