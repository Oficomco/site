    ...
    openSection: function(section) {
        ...
        if(section.id != this.currentSection) {
            ...
            if ( $('checkoutSteps') !== null ) {
                var isHeaderFixed = $$('.menu-contain')[0].hasClassName('fixed'),
                    prevSibling = Element.select(section, '.step-title'),
                    headerHeight = Element.getHeight($$('.menu-contain')[0]),
                    header2Height = Element.getHeight($$('.header2')[0]);
                    
                if ( !isHeaderFixed ) {
                    headerHeight += header2Height;
                }    
                Effect.ScrollTo( prevSibling[0], { offset: -headerHeight, duration: .3 } ); // 57px is the current height of the sticky header, duration is 300 milliseconds
            }
